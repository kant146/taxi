...
export default function AppCommon({ children }) {
  ...
  useEffect(() => {
    if (auth.info && auth.info.profile && auth.info.profile.usertype == 'driver' && tasks && tasks.length > 0 && Platform.OS == 'ios') {
      notifyBooking();
    }
  }, [auth.info, tasks]);
      ...
      if(!playedSounds.includes(tasks[i].id)){
        await Notifications.scheduleNotificationAsync({
          content: {
            ...
            sound: settings.CarHornRepeat? 'repeat.wav': 'horn.wav'
          },
          trigger: null,
        });
  ...
  const saveToken = async () => {
    ...
    if((auth && auth.info && auth.info.profile && auth.info.profile.pushToken && auth.info.profile.pushToken != token) || !auth.info.profile.pushToken ){
      dispatch(
        api.updatePushToken(
          auth.info,
