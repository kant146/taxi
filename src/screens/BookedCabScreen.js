        ...
        if (activeBookings && activeBookings.length >= 1) {
            ...
            if (booking) {
                ...
                if (booking.status == 'NEW' && (booking.bookLater == false || (booking.bookLater && diffMins <= 15))) {
                    ...
                    if (role == 'rider' && appcat == 'rentals' && booking.selectedBid && !booking.customer_paid){
                        setTimeout(()=>{
            ...
            (curBooking && role == 'rider' && (curBooking.status == 'NEW' || curBooking.status == 'ACCEPTED')) ||
                ...
                <View style={{ flex: 1.5, flexDirection: 'row' }}>
                    ...
                    {appcat == 'delivery' && settings.AllowDeliveryPickupImageCapture && role == 'driver' && !curBooking.pickup_image && (curBooking.status == 'ACCEPTED' || curBooking.status == 'ARRIVED') ?
                        ...
                        : null}
                    {role == 'driver' && ((appcat == 'rentals') || (curBooking.pickup_image && appcat == 'delivery') || (!settings.AllowDeliveryPickupImageCapture && appcat == 'delivery') || appcat == 'taxi') && (curBooking.status == 'ACCEPTED' || curBooking.status == 'ARRIVED') ?
                        <View style={{ flex: 1 }}>
                            <Button
                                title={t('start_trip')}
                                loading={false}
                    ...
                    {appcat == 'delivery' && settings.AllowFinalDeliveryImageCapture && role == 'driver' && !curBooking.deliver_image && curBooking.status == 'STARTED' ?
                        ...
                        : null}
                    {role == 'driver' && ((appcat == 'rentals') || (curBooking.deliver_image && appcat == 'delivery') || (!settings.AllowFinalDeliveryImageCapture && appcat == 'delivery') || appcat == 'taxi') && curBooking.status == 'STARTED' ?
                        <View style={{ flex: 1 }}>
                            <Button
            ...
            >
                <View style={{ flex: 1, backgroundColor: colors.BACKGROUND, justifyContent: 'center', alignItems: 'center' }}>
                    ...
                    <View style={{ width: width - 40, backgroundColor: colors.WHITE, borderRadius: 10, flex: 1, maxHeight: height - 200,marginTop:15 }}>
                        ...
                        <View style={{marginTop: 60,width: width - 80, height: height - 340, marginRight:10,marginLeft:10, alignSelf:'center'}}>
                            <ScrollView showsVerticalScrollIndicator={false} style={{ flex:1}}>
                                {Object.keys(curBooking.driverOffers).map(key=>  
                                    <View key={key} style={{height: 90, marginBottom: 20,borderColor: 'black', borderWidth: 2, flexDirection:'row', borderRadius: 10}}>
                                        <View style={{width: width - 190, marginTop:8, marginBottom:8, flexDirection: 'column', justifyContent: 'space-evenly', alignItems:'center'}} >
                                            <Text style={{ color: colors.BLACK, fontSize: 16 }}>{curBooking.driverOffers[key].driver_name}</Text>
                                            <Text style={{ color: colors.BLACK, fontSize: 22, fontWeight: '700' }}>{settings.symbol} {curBooking.driverOffers[key].trip_cost}</Text>
                                            <Text style={{ color: colors.BLACK, fontSize: 12 }}>{moment(curBooking.driverOffers[key].deliveryDate).format('lll')}</Text>
                                        </View>
                                        <View style={{width: 90, justifyContent:'center', alignContent:'center'}}>
                                            <Button
                                                ...
                                                buttonStyle={{width:90, backgroundColor: colors.GREEN_DOT}}
                                            />
                                        </View>
                                    </View>
                    ...
                    :
                    <View style={{ width: width - 40, backgroundColor: colors.WHITE, borderRadius: 10, flex: 1, maxHeight: 310,marginTop:15 }}>
                        <Image source={require('../../assets/images/lodingDriver.gif')} resizeMode={'contain'} style={{ width: 160, height: 160, marginTop: 30, alignSelf:'center' }} /> 
                        <View style={{ color: colors.BLACK, alignSelf:'center' }}>
                            <Text style={{ color: colors.BLACK, fontSize: 16, marginTop: 12, marginBottom:50 }}>{t('driver_assign_messege')}</Text>
                        </View>
                        <View style={{position:'absolute', bottom:20,  alignSelf:'center'}}>
                            <Button
                                title={t('close')}
            ...
            <View style={styles.mapcontainer}>
                ...
                {curBooking && !(curBooking.status == 'NEW') ?
                    <TouchableOpacity
                        ...
                        onPress={() => role == 'rider' ? onPressCall(curBooking.driver_contact) : ((appcat == 'taxi' || appcat == 'rentals') ? onPressCall(curBooking.customer_contact) : setUserInfoModalStatus(true))}
                    >
                        <Icon
                            name="ios-call"
                            type="ionicon"
...
const styles = StyleSheet.create({
    ...
    otpText: { alignSelf: 'center', color: colors.BLACK, fontFamily: 'Roboto-Bold', marginLeft: 70 },
