...
import { Text, View, StyleSheet, Dimensions, FlatList, Modal, TouchableHighlight, TouchableWithoutFeedback, Switch, Image, Platform, TextInput } from 'react-native';
...
export default function DriverTrips(props) {
    ...
    const onPressAccept = (item) => {
        ...
        if (!settings.negativeBalance && wallet_balance <= 0) {
        ...
        else if(!settings.negativeBalance && wallet_balance > 0 && wallet_balance < item.convenience_fees && !(appcat === 'rentals')){
            if(appcat == 'delivery' && item.prepaid && item.payment_mode == 'card'){
                dispatch(acceptTask(auth.info, item));
                setSelectedItem(null);
                setModalVisible(null);
            ...
            if(parseFloat(amount) > 0){
                ...
                let convenience_fees = item.commission_type == 'flat'? parseFloat(item.commission_rate) :  (parseFloat(amount) * parseFloat(item.commission_rate)/100);
                let obj = {};
                    ...
                    return (
                        <View style={styles.listItemView}>
                            ...
                            <View style={styles.mapDetails}>
                                ...
                                {appcat == 'rentals' && item.driverOffers && item.driverOffers[auth.info.uid]?
                                <View style={styles.rateViewStyle}>
                                    {settings.swipe_symbol===false?
                                        <Text style={styles.rateViewTextStyle}>{settings.symbol}{item.driverOffers[auth.info.uid].driver_share}</Text>
                                        :
                                        <Text style={styles.rateViewTextStyle}>{item.driverOffers[auth.info.uid].driver_share}{settings.symbol}</Text>
                                    }
                                </View>
                                :null}
                                ...
                                :null}
                                {appcat == 'rentals'?
                                <View style={[styles.textContainerStyle, {paddingBottom: 15,flexDirection: isRTL? 'row-reverse' : 'row'}]}>
                                ...
                                :null}
                                {appcat == 'rentals' && item.status == 'NEW' && !(item.driverOffers && item.driverOffers[auth.info.uid])?
                                 <View style={styles.box}>
                                    <TextInput
                                        style={[styles.dateTextStyle,{ textAlign: isRTL ? "right" : "left" }]}
                                        placeholder={t('amount')}
                                        onChangeText={(value) => setAmount(value)}
                                        value={amount && !isNaN(amount)?amount.toString():null}
                                ...
                                {activeBookings && activeBookings.length >= 1 ?
                                    ...
                                    <View style={[styles.detailsBtnView,{flexDirection: isRTL? 'row-reverse' : 'row'}]}>
                                        ...
                                        {!(appcat === 'rentals') || (appcat == 'rentals' && !(item.driverOffers && item.driverOffers[auth.info.uid]))?
